...
DEFINE TEMP-TABLE ttOrderLine NO-UNDO LIKE Order-line.

DEFINE DATASET dsOrder FOR ttOrder, ttOrderLine
    DATA-RELATION drOrderLine FOR ttOrder, ttOrderLine NESTED
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE vNumOrdersToPull AS INTEGER     NO-UNDO.

    ASSIGN vNumOrdersToPull = INT(GetParVal("NumOrdersToPull",iInputPars)) NO-ERROR.
    FOR EACH Order NO-LOCK i = 1 TO vNumOrdersToPull:
        CREATE ttOrder.
        BUFFER-COPY Order TO ttOrder.
        FOR EACH Order-line OF Order NO-LOCK:
            CREATE ttOrderLine.
            BUFFER-COPY Order-line TO ttOrderLine.
        END.
    END.
    ASSIGN oOutputPars = "This is the order/order-line dataset"
           oErrMsg = "This is a sample error message"
           .
END PROCEDURE.
